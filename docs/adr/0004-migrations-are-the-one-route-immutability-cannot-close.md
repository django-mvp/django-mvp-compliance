...
One route stays open: a data migration shipped by this package calling `save()` on a historical instance. It is closed by this package shipping no such migration, and by a test asserting that no migration it ships contains a `RunPython` or `RunSQL` operation.
...
The route that remains is not one the package offers anyone. It is reachable only by code inside this repository, written by the people who set the rule, reviewed like any other change, and now covered by a test that fails the moment someone adds a data migration.
...
This package genuinely needs to ship a data migration. The test will fail, which is the point: the migration then has to be shown not to write to a published version, and this ADR gets the exception recorded rather than assumed.

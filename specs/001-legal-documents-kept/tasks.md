...
| T022 | `mvp_compliance/models.py` — `Version.Status` (`draft`, `current`, `superseded`), the `status` field, `published_at` (`DateTimeField(null=True, blank=True, editable=False)`), `is_published`, and a `CheckConstraint` tying `status` and `published_at` to each other: a draft carries no publication time, a published version carries one. `html` is US-4's, and US-4 widens the constraint to cover it | T020 passes |
...
| T024 | `mvp_compliance/models.py` — `Version.publish()` per plan.md: refuse when not a draft, lock the document row with `select_for_update()`, supersede the current version, become current, all inside `transaction.atomic()`. A comment at the lock says why it is not belt-and-braces: on MySQL and MariaDB the partial constraint in T025 is silently omitted and this lock is the only thing holding FR-007 (`research.md` R1, D11) | T021 passes |
...
| T031 | `mvp_compliance/models.py` — `PUBLISHED_FROZEN_FIELDS`, the `save()` guard reading the stored row rather than an in-memory snapshot, and `VersionQuerySet.update()`, `.delete()` plus `Version.delete()`, all raising `PublishedVersionError`. No `bulk_update()` override: Django's `bulk_update` calls `.update()` internally, so the `update()` guard already catches it. Compare each frozen field by `name` **and** `attname`, or `document_id` slips past | T030 passes, including its `bulk_update` case |
...
| T044 | `mvp_compliance/models.py` — `html` (`TextField(blank=True)`), `publish()` renders into it before anything is written, and T022's check constraint widened so a draft's `html` is empty and a published version's is not (FR-019, scenario 5) | `tests/test_models.py::TestPublishing` covers both |

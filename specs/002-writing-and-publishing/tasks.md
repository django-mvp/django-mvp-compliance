...
| T041a | `mvp_compliance/templates/admin/mvp_compliance/version/change_form.html` — a **Publish** link in the change form's object tools, rendered only for a draft and only to a caller holding `publish_version`. Forecast by T035's row and built here: FR-013 says the confirmation page is the only route to publication, which presupposes a route to reach the confirmation page. A link is not an action, a field or a changelist option, so it adds no second way to publish | `tests/test_admin.py::TestPublish` asserts the link appears for a permitted caller and is withheld from one without the permission |

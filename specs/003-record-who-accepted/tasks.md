...
| T017 | `mvp_compliance/models.py` — `AcceptanceQuerySet` refusing `update()` whenever it matches anything and refusing `delete()` outright; `Acceptance.save()` refusing when the database already holds the row, asked of the database rather than of Django's adding flag; `Acceptance.delete()` refusing. `AcceptanceManager.use_in_migrations = True`, and the manager forwards each queryset method it needs explicitly (D21's reason, and the shape `VersionManager` already uses) | T016 passes |

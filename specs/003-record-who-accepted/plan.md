...
check that does not fire. It also has the pair of changes that would route the resulting field update
into `AcceptanceQuerySet.update()`'s refusal instead of the raw update the collector otherwise
issues: a `lazy_sub_objs` attribute on the callable, which Django's own `SET_NULL` carries, together
with `Meta.base_manager_name` naming `AcceptanceManager`. Neither alone does anything, which is
exactly why the callable carries a comment about both — the only thing stopping a later cleanup pass
from reintroducing them is knowing what they do together.
